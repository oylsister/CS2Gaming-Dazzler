...
namespace Dazzler
{
    ...
    {
        public override string ModuleName => "The Dazzler Acheivement";
        ...
        public Dictionary<CCSPlayerController, PlayerFlashCount> _playerFlashCount { get; set; } = new();
        ...
        public HookResult OnPlayerConnectFull(EventPlayerConnectFull @event, GameEventInfo info)
        {
            ...
            if (data == null)
                _playerFlashCount.Add(client!, new());

            else
            {
                var count = data.FlashCount;
                ...
                _playerFlashCount.Add(client!, new(count, complete));
            }
        ...
        public void OnClientDisconnect(int playerslot)
        {
            ...
            var steamID = client!.AuthorizedSteamID!.SteamId64;
            var complete = _playerFlashCount[client].Complete;
            var value = _playerFlashCount[client].FlashCount;

            Task.Run(async () => await SaveClientData(steamID, value, complete, !complete));

            _playerFlashCount.Remove(client!);
        }
        ...
        public HookResult OnPlayerBlind(EventPlayerBlind @event, GameEventInfo info)
        {
            ...
            if (client!.TeamNum != attacker!.TeamNum)
        ...
        public void CountKill(CCSPlayerController client)
        {
            ...
            if (Config.MaxFlashbangCount <= 0)
                return;

            if (!_playerFlashCount.ContainsKey(client!))
                return;

            if (_playerFlashCount[client!].Complete)
                return;

            _playerFlashCount[client!].FlashCount += 1;

            if (_playerFlashCount[client!].FlashCount >= Config.MaxFlashbangCount)
            {
                var steamid = client.AuthorizedSteamID?.SteamId64;
        ...
        public async Task TaskComplete(CCSPlayerController client, ulong steamid)
        {
            ...
            _playerFlashCount[client].Complete = true;
            ...
            if (response != null)
            {
                ...
                Server.NextFrame(() =>
                {
                    client.PrintToChat($" {ChatColors.Green}[Acheivement]{ChatColors.Default} You acheive 'The Dazzler' (Blinds enemies with flashbang for {Config.MaxFlashbangCount} times)");
                    client.PrintToChat($" {ChatColors.Green}[Acheivement]{ChatColors.Default} {response.Message}");
                });

                await SaveClientData(steamid!, _playerFlashCount[client].FlashCount, true, true);
            }
        }
        ...
        private async Task SaveClientData(ulong steamid, int count, bool complete, bool settime)
        {
            ...
            {
                jsonObject[steamKey].FlashCount = count;
